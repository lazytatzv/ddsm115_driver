...
: serial_port_(io_context_), buffer_(), reading_(false), feedback_callback_(feedback_callback),
...
void DDSM115DriverClient::send_mode_command(uint8_t motor_id, ControlLoopModes mode)
{
  ...
  std::stringstream ss;
  ss << "Sending mode command to motor " << static_cast<int>(motor_id) << ": mode " <<
...
bool DDSM115DriverClient::send_current_command(uint8_t motor_id, double current_amps)
{
  ...
  int16_t mapped_val = static_cast<int16_t>(std::clamp(std::round(current_amps * (32767.0 / 8.0)),
      -32767.0, 32767.0));
...
bool DDSM115DriverClient::send_velocity_command(
  uint8_t motor_id, double rpm, uint8_t acceleration,
  bool brake)
{
  last_sent_command_type_[motor_id] = 0x64;
  std::vector<uint8_t> data(10, 0x00);
...
{
  // Motor ID setting command does not target a specific current ID.
  // It must be run with ONLY ONE motor connected to the bus.
  (void)current_id;
  std::vector<uint8_t> data(10, 0x00);
  data[0] = 0xAA;
  data[1] = 0x55;
...
bool DDSM115DriverClient::is_valid_motor_id(uint8_t motor_id) const
{
  ...
  return std::find(registered_motor_ids_.begin(), registered_motor_ids_.end(),
      motor_id) != registered_motor_ids_.end();
}
